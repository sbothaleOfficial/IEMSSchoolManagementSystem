...
public partial class StudentDocumentsWindow : Window
{
    private readonly StudentDocumentService _service;
    private readonly StudentDto _student;
    private readonly string _currentUser;

    public StudentDocumentsWindow(StudentDto student, StudentDocumentService service, string currentUser)
    {
        InitializeComponent();
        _student = student;
        _service = service;
        ...
        lblTitle.Text = $"Documents — {student.FullName}  ({student.StudentNumber})";
        foreach (var t in StudentDocumentService.DocumentTypes) cmbDocType.Items.Add(t);
    ...
    {
        var docs = await _service.GetDocumentsAsync(_student.Id);
        dgDocs.ItemsSource = docs;
        lblStatus.Text = docs.Count == 0
    ...
    private void AddDocument(string fileName, string contentType, byte[] bytes)
    {
        ...
        {
            await _service.AddDocumentAsync(_student.Id, SelectedType, fileName, contentType, bytes, _currentUser);
            await LoadAsync();
    ...
    private void OpenSelected()
    {
        ...
        AsyncHelper.SafeFireAndForget(async () =>
        {
            ...
            var temp = Path.Combine(Path.GetTempPath(), "IEMS_doc_" + Path.GetFileNameWithoutExtension(safe) + ext);
